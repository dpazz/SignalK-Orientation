...
{ 
    ...
    pStatus->update = (ssUpdateStatus_t*) ssUpdateStatus;
    pStatus->test = (ssUpdateStatus_t*) ssTest;
                             ...
                             )
{
    ...
    sfg->isFusionStarted = false;             // used when  F_USING_SMARTSENSOR to track end of fusion start
...
#endif
#if F_USING_TEMPERATURE
    sfg->Temp.iWhoAmI = 0;
#endif
#ifdef F_USING_SMARTSENSOR  // zero calib data in sfg
int8_t i;
    for (i = 0; i < 3; i++)
    {
            sfg->AccelCal.fV[i] = sfg->MagCal.fV[i] = sfg->GyroCal.fV[i] = 0;
    }
...
{
    sfg_ = &sfg;                            //use of static data area instead of heap
    status_subsystem_ = &status_subsystem;  //use of static data area instead of heap
    num_sensors_installed_ = 0;
                                 ...
                                 SensorType sensor_type)
{
    ...
    case SensorType::kAccelerometer:
            sfg_->installSensor(sfg_, &sensors[num_sensors_installed_],
    ...
    case SensorType::kThermometer:
            // use the thermometer built into Combo as if it was a separate sensor. Not precise nor calibrated,
            // but OK. 
            sfg_->installSensor(sfg_, &sensors[num_sensors_installed_],
...
void SensorFusion::RunFusion(void)
{
    ...
    if (0 == sfg_->loopcounter % 4)
    {
            ...
            if (0 == sfg_->loopcounter % 16){
                Get_NDOF_Calib(sfg_);        // update sfgglobals with the calib data
                                             // calculated by smartsensor MCU
                                             // at 1/4 the pace of status updates
            }*/
            #endif
    }
...
/**
 * @brief Save current magnetic (or all_sensors in case of smartsensor)
 *        calibration to non-volatile memory.
 *
 ...
 * (this not in case of smartsensor)
 *
...
void SensorFusion::SaveMagneticCalibration()
{
    SaveMagneticCalib(sfg_);
...
#endif
#ifdef F_USING_SMARTSENSOR
    EraseMagneticCalib(sfg_);
#endif
} // end EraseMagneticCalibration()
...
/*
    return (sfg_->SV_9DOF_GBY_KALMAN.fRhoPl > 90)
            ...
            :(sfg_->SV_9DOF_GBY_KALMAN.fRhoPl + 270) ;// smartsensor output an already conditioned heading in degrees*/
...
{
#ifdef F_USING_SMARTSENSOR
    return sfg_->SV_9DOF_GBY_KALMAN.fAccGl[0];
#endif
#ifndef F_USING_SMARTSENSOR
...
{
#ifdef F_USING_SMARTSENSOR
    return sfg_->SV_9DOF_GBY_KALMAN.fAccGl[1];
#endif
#ifndef F_USING_SMARTSENSOR
...
{
#ifdef F_USING_SMARTSENSOR
    return sfg_->SV_9DOF_GBY_KALMAN.fAccGl[2];
#endif
#ifndef F_USING_SMARTSENSOR
...
#ifdef F_USING_SMARTSENSOR
/**
 * @brief @return Return the computed index of calibration for IMU chip [0..3]
 *
 * The index is the quality of autocalibration process
 * 0 means no calibration reached; 3 fully calibrated.
 */
uint8_t SensorFusion::GetIMUCalibrationIndex(void)
{
    return sfg_->IMU_Calib_index;
} // end GetIMUCalibrationIndex()
/**
 * @brief @return Return the computed index of calibration for Magnetometer [0..3]
 *
 * The index is the quality of autocalibration process
 * 0 means no calibration reached; 3 fully calibrated.
 */
uint8_t SensorFusion::GetMagCalibrationIndex(void)
{
    return sfg_->MagCal.Mag_Calib_index;
} // end GetMagCalibrationIndex()
/**
 * @brief @return Return the computed index of calibration for Accelerometer [0..3]
 *
 * The index is the quality of autocalibration process
 * 0 means no calibration reached; 3 fully calibrated.
 */
uint8_t SensorFusion::GetAccelCalibrationIndex(void)
{
    return sfg_->AccelCal.Accel_Calib_index;
} // end GetAccelCalibrationIndex()
/**
 * @brief @return Return the computed index of calibration for Accelerometer [0..3]
 *
 * The index is the quality of autocalibration process
 * 0 means no calibration reached; 3 fully calibrated.
 */
uint8_t SensorFusion::GetGyroCalibrationIndex(void)
{
    return sfg_->GyroCal.Gyro_Calib_index;
} // end GetGyroCalibrationIndex()

void SensorFusion::GetCalibrationData(int16_t *Cal_data)
{

    int8_t i;
    for (i = 0; i < 3; i++)
    {
            Cal_data[i] = sfg_->AccelCal.fV[i];
            Cal_data[i + 3] = sfg_->MagCal.fV[i];
            Cal_data[i + 6] = sfg_->GyroCal.fV[i];
    }
    Cal_data[9] = sfg_->AccelCal.Radius; // accel radius
    Cal_data[10] = sfg_->MagCal.Radius;  // mag radius

} // end GetCalibrationData()

void SensorFusion::SetCalibrationData(int16_t *Cal_data)
{

    int8_t i;
    for (i = 0; i < 3; i++)
    {
            sfg_->AccelCal.fV[i] = Cal_data[i];
            sfg_->MagCal.fV[i] = Cal_data[i + 3];
            sfg_->GyroCal.fV[i] = Cal_data[i + 6];
    }
    sfg_->AccelCal.Radius = Cal_data[9]; // accel radius
    sfg_->MagCal.Radius = Cal_data[10];  // mag radius

} // end SetCalibrationData()

void SensorFusion::ReadCalibrationData()
{
    Get_NDOF_Calib(sfg_);
} // end SetCalibrationData()

void SensorFusion::SetAxisRemap(physical_sensor_position_t // changes the reference coordinates of the IMU chip
                                    pos)
{
    SetAxisremap(sfg_, pos);
} // end SetAxisremap
#endif
