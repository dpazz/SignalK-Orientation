...
#define BOARD_COMBO_AMG_I2C_ADDR    (BNO055_ADDRESS_B) ///< I2C address on breakout board (for Bosh BNO055)
#define BOARD_COMBO_BARO_THERM_I2C_ADDR    (0x00) ///< I2C address on breakout board (for BMP280)
...
//                                  Accelerometer/Magnetometer/Gyroscope single sensor
//                          If using combo sensors the first position is for Combo Accel/Mag or
//                                  Combo Accel/MAg/Gyro
//                          the fourth position is reserved for a Combo Baro/Therm sensor (like BMP280)

const orsensor_bus_addresses bus_i2c_addr = {BOARD_COMBO_AMG_I2C_ADDR,0,0,BOARD_COMBO_BARO_THERM_I2C_ADDR};
const sensor_cap sensor_capability = AMGT;
IPAddress staticIP (10,10,10,3);
...
void setup() {
  ...
  SensESPAppBuilder builder;
   if (WiFi.config(staticIP, gateway, subnetmask, dns1, dns2) == false) {
    Serial.println("Fixed IP configuration failed."); delay(5000);
  }
  sensesp_app = (&builder)
  ...
  const char* kSKPathMagCalValues   = "orientation.calibration.indexes_and_values";
  #endif
  /**
   ...
   * 
       sensors->attitude
                       ...
                       ->offset     (stores r/w the calib data for acc/gyro/mag subsensor)
                                    (N.B.: to be loaded on startup to shorten the calib duration)
              ->heading
  ...
  const char* kConfigPathAttitudeOffset    = "/sensors/attitude/offset";
  ...
  const char* kConfigPathMagCal = "/sensors/mag_or_all/calibrate";
  const char* kConfigPathMagCal_SK = "/sensors/mag_or_all/sk";
  ...
  auto* orientation_sensor = new OrientationSensor(
      PIN_I2C_SDA, PIN_I2C_SCL, bus_i2c_addr,sensor_capability);
...
#ifdef F_USING_SMARTSENSOR
/**
   * The preceding outputs were useful when calibrating in host system. See the wiki at
   * @see https://github.com/BjarneBitscrambler/SignalK-Orientation/wiki
   ...
   * need to be sent to.
   * 
   * When we are using a smartsensor with an MCU executing itself the calibration
   * the info on calibration status is contained in some calibration indexes
   * ranging from 0 (not calibrated) to 3 (fully calibrated) for each
   * subsystem involved in the 9DOF fusion:
   *    IMU_Calib_index  ---> the calib status of the system itself
   *    Gyro_Calib_index ---> the calib status of gyroscope sensor
   *    Accel_Calib_index---> the calib status of accelerometer
   *    Mag_Calib_index  ---> the calib status of magnetometer
   *
